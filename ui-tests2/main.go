...
func testHome(u string) {
	page := rod.New().ControlURL(u).MustConnect().MustPage("http://localhost:3000/")

	// Test hero title
	title := page.MustElement(".hero-title")
	assertEquals(title.MustText(), "Decentralized Cloud Storage is Here", "The title is not present. Perhaps the homepage is not opening")
...
func main() {
	// Remove all of the contents within the output directory
